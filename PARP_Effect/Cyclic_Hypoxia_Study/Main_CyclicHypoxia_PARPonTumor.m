...
% between the three different modules. This includes the effects of PARP on
...
%Model parameters
global delta1 alfa1 delta2 alfa2 alfam2 kAng2 gamma1 gamma2 degAng2 dos Pj nj
...
while T<Tmax
   ...
   %%%Third step of the time splitting: pTie2
   ...
   par_4step=[k_C,K_C,K_E,th_pT,k_E,th_SH,th_H,th_x,k_proO2,k_conO2,d_O2,d_C,k_cte,d_cte,xstar(step-1),Pj,nj,dt];
   
   K1=RK4_4step_AngCell_xstarPjC(par_4step,X_CelMod(1,1:4),pTie2(step));
   X1=X_CelMod(1,1:4)+K1./2;
   
   K2=RK4_4step_AngCell_xstarPjC(par_4step,X1,pTie2(step));
   X2=X_CelMod(1,1:4)+K2./2;
    
   K3=RK4_4step_AngCell_xstarPjC(par_4step,X2,pTie2(step));
   X3=X_CelMod(1,1:4)+K3;

   K4=RK4_4step_AngCell_xstarPjC(par_4step,X3,pTie2(step));
   ...
   %%%Fifth step of the time splitting: VEGF module
   ...
   if mod(step,1000)==0
   save('AdvancedStage_PARPonTumor_partial.mat')
   end
   
end 

%Saving option for final results
save('AdvancedStage_PARPonTumor_final.mat')
